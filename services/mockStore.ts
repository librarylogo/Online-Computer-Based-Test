...
let MOCK_SETTINGS: AppSettings = {
  ...
  schoolLogoUrl: 'https://lh3.googleusercontent.com/d/1tQPCSlVqJv08xNKeZRZhtRKC8T8PF-Uj?authuser=0', // Default Logo (App Logo) for Card Printing
...
export const mockDb = {
  ...
  login: async (input: string, password?: string): Promise<User | undefined> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const user = MOCK_USERS.find(u => u.username === input || u.nomorPeserta === input);
        if (user) {
